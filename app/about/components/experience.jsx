...
function ExperienceItem({ experience, index }) {
	...
	return (
		<div className="relative">
			{/* Timeline dot */}
			<div className="absolute w-5 h-5 bg-gray-900 rounded-full border-4 border-white shadow-lg z-30 md:left-1/2 md:-translate-x-1/2 md:top-8 left-0 -translate-x-1/2 top-5" />
			...
			>
				<div className="bg-white border border-neutral-200 shadow-sm rounded-2xl p-4 sm:p-6 hover:shadow-md transition-all duration-300 ml-10 md:ml-0">
					<div className="mb-3 sm:mb-4">
						<h3 className="font-bold text-base sm:text-xl text-gray-900 mb-1">{experience.company}</h3>
						<h4 className="font-medium text-sm sm:text-lg text-gray-700">
							{experience.position}
							<span className="text-xs sm:text-sm font-normal text-gray-500 ml-2">• {experience.type}</span>
						</h4>
					</div>
					<p className="text-gray-600 text-sm leading-relaxed mb-4">{experience.description}</p>
